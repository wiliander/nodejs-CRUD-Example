...
function findAll(callback) {
    pool.connect(function (err, client, done)  {
        ...
        client.query('SELECT * FROM matricula', [], function (err, result) {
            debugger;
            if (err) {
                console.log(err);
                callback([])
...
function excluir(callback, id) {
    pool.connect(function (err, client, done) {
        ...
        client.query('DELETE FROM matricula WHERE id_matricula = $1', [id], function (err, result) {
            debugger;
            if (err) {
                console.log(err);
                callback(err)
...
function insert(callback, matricula) {

    const sql = "INSERT INTO matricula(data_matricula, id_aluno, id_curso) VALUES($1, $2, $3) RETURNING *";
    const values = [matricula.datamatricula, matricula.idaluno,matricula.idcurso];
